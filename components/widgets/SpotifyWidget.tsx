...
export default function SpotifyWidget({ widget, isOwner }: Props) {
  const connected = !!(widget?.connected_at)

  return (
    <WidgetCard label="Spotify" connected={connected} isOwner={isOwner}>
      <div className="flex flex-col items-center justify-center h-32 gap-3">
        <SpotifyMark className="text-muted" />
        {!connected ? (
          isOwner ? (
          ...
          <p className="text-[11px] text-muted">Spotify connected</p>
        )}
      </div>
    </WidgetCard>
  )
